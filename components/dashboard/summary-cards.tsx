...
export default function SummaryCards({ transactions }: Props) {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 sm:gap-4 md:gap-5 lg:gap-6 mb-6 sm:mb-8 md:mb-10 animate-fade-in-up">
        ...
        <CardContent className="p-4 sm:p-6 md:p-8 lg:p-10 xl:p-12">
          <div className="space-y-6">
            ...
            <div className="space-y-3">
              <div className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl xl:text-6xl 2xl:text-7xl font-bold text-red-600 dark:text-red-400 group-hover:text-red-700 dark:group-hover:text-red-300 transition-colors break-words">
                ₦{totalDebit.toLocaleString()}
              </div>
              <p className="text-xs sm:text-sm md:text-base text-gray-600 dark:text-gray-400 group-hover:text-gray-700 dark:group-hover:text-gray-300 transition-colors">{debitCount} transaction{debitCount !== 1 ? 's' : ''} this period</p>
            </div>
          </div>
        </CardContent>
      </Card>
      ...
      <Card className="lg:col-span-1 xl:col-span-2 bg-gradient-to-br from-white/50 to-white/30 dark:from-white/10 dark:to-white/5 backdrop-blur-xl border border-white/20 dark:border-white/10 hover:border-white/40 dark:hover:border-white/20 hover:bg-gradient-to-br hover:from-white/60 hover:to-white/40 dark:hover:from-white/15 dark:hover:to-white/10 shadow-lg hover:shadow-xl hover:shadow-green-500/10 dark:hover:shadow-green-500/5 transition-spring group cursor-default">
        <CardContent className="p-4 sm:p-6 md:p-8 lg:p-10 xl:p-12">
          <div className="space-y-6">
            ...
            <div className="space-y-3">
              <div className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl xl:text-6xl 2xl:text-7xl font-bold text-green-600 dark:text-green-400 group-hover:text-green-700 dark:group-hover:text-green-300 transition-colors break-words">
                ₦{totalCredit.toLocaleString()}
              </div>
              <p className="text-xs sm:text-sm md:text-base text-gray-600 dark:text-gray-400 group-hover:text-gray-700 dark:group-hover:text-gray-300 transition-colors">{creditCount} transaction{creditCount !== 1 ? 's' : ''} this period</p>
            </div>
          </div>
        </CardContent>
      </Card>
